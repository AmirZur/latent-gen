...
def create_dataset(
    ...
    for cf in tqdm(edit_dataset):
        # make sure cf[aspect_key] == edit_goal == aspect_value
        if cf[aspect_key] != cf['edit_goal'] or cf[aspect_key] != aspect_value:
            continue

        original_id = cf['original_id'] + '000000' if cf['original_id'] != '0' else '0'
        base = df[df['id'] == original_id].iloc[0]
        source = df[df[aspect_key] == cf[aspect_key]].sample().iloc[0]
